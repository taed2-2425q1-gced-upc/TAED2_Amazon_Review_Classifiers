...
def test_model_robustness_original_vs_negated(model_and_tokenizer):
    """" Test the model's robustness against negation in reviews. """
    model, tokenizer = model_and_tokenizer

    negation_reviews = [
        ("The product works perfectly and arrived quickly.",
         "I wouldn't say the product works bad, and it didn't arrive late."),
        ("The subscription is expensive and not worth it.",
         "I wouldn't say the subscription is cheap, and it definitely wasn't worth it."),
        ("The product is affordable and does the job well.",
         "The product is not expensive, I can afford it and does a good job."),
        ("Great value for the money, highly recommend!",
         "Not a bad price for this product, highly recommended!"),
        ("Not worth the price, very disappointing quality.",
         "I thought it was worth the price. It wasn’t. The quality is disappointing.")
    ]
    ...
    for original_review, negated_review in negation_reviews:
        ...
        _, prob_neg = predict_sentiment(negated_review, model, tokenizer)
    ...
    if failures:
        ...
        f"{len(failures)} out of {len(results)} tests failed in the Negation robustness tests:\n" +
